...
export default function StatsPage() {
  ...
  const masteredQuestions = Object.values(progress).filter(p => p.correctCount >= 4).length;
    ...
    <main className="stats-page">
      <div className="stats-container">
        ...
        <div className="stats-tabs">
          <Link href={withBase('/stats')} className="stats-tab active">全体</Link>
          {Object.entries(EXAM_INFO).map(([slug, info]) => (
            <Link key={slug} href={withBase(`/${slug}/stats`)} className="stats-tab">
              {info.shortName}
            </Link>
          ))}
        </div>
